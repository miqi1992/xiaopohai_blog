...
(  
    `id`          int          NOT NULL AUTO_INCREMENT,  
    `yhq_id`      int          DEFAULT NULL COMMENT '优惠券id',  
  ...
  DEFAULT CHARSET = utf8mb3;
-- 充值卡变更记录
CREATE TABLE `recharge_card_change_record`  
(  
    `id`      int          NOT NULL AUTO_INCREMENT,  
    `yhq_id`   int          NOT NULL COMMENT '优惠券id',  
  ...
  DEFAULT CHARSET = utf8mb3;
-- 充值卡使用记录
CREATE TABLE `recharge_card_use_record`  
(  
    `id`      int          NOT NULL AUTO_INCREMENT,  
    `yhq_id`   int          NOT NULL COMMENT '优惠券id',  
    `card_id`    varchar(255) NOT NULL COMMENT '兑换卡id',  
    `value`    int(10) NOT NULL COMMENT '金额',  
...
(  
    ...
    `merchant_id`   int(10)            NOT NULL COMMENT '集团id',  
    `merchant_name` varchar(255)   NOT NULL COMMENT '商家名字',  
    `value`        int(10) NOT NULL COMMENT '支付金额',  
    `status`      int            NOT NULL COMMENT '状态0未取出，1审核中，2已取出',  
...
insert into recharge_card (id, yhq_id, code, passwd, face_value, status, create_user, create_time, update_user, update_time)  
select id, yhqid, kahao, pswd, COALESCE(CAST(NULLIF(jine, '') AS DECIMAL(10, 2)), 0) * 100, status, 'system',  
       FROM_UNIXTIME(COALESCE(NULLIF(time, ''), UNIX_TIMESTAMP())), 'system', FROM_UNIXTIME(COALESCE(NULLIF(time, ''), UNIX_TIMESTAMP()))  
from sqsywl_2024.ims_zt_haibao_chongzhi_ka;
...
insert into recharge_card_use_record (id, yhq_id, card_id, value, user_id, type, create_user, create_time,  
...
update recharge_card_use_record set type = 2 where type = 1;
update recharge_card_use_record set type = 1 where type = 0;


-- 餐卡
insert into meal_card (id, yhq_id, code, passwd, face_value, status, user_id, create_user, create_time, update_user, update_time)  
select id, yhqid, kahao, pswd, COALESCE(CAST(NULLIF(jine, '') AS DECIMAL(10, 2)), 0) * 100, status, uid, 'system',  
       FROM_UNIXTIME(COALESCE(NULLIF(time, ''), UNIX_TIMESTAMP())), 'system', FROM_UNIXTIME(COALESCE(NULLIF(time, ''), UNIX_TIMESTAMP()))  
from sqsywl_2024.ims_zt_liang_canka_item;


-- 餐卡变更记录
insert into meal_card_change_record (id, yhq_id, card_id, old_value, new_value, create_user, create_time, update_user, update_time)  
select id,  
       yhqid,  
       kaid,  
       COALESCE(CAST(NULLIF(old, '') AS DECIMAL(10, 2)), 0) * 100,  
...
-- 账户记录
insert into account_record (id, user_id, group_id, merchant_id, merchant_name, value, status, type, create_user, create_time, update_user, update_time)  
